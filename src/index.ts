...
// This is critical for Paystack webhooks which sign the exact raw bytes
app.use(express.json({ 
  limit: '50mb',
  verify: (req: any, res, buf) => {
    // Store raw body as string for webhook signature verification
    // Only for webhook routes that need to verify signatures
...
(async () => {
  ...
  server.listen({
    ...
    // Log configured payment gateways
